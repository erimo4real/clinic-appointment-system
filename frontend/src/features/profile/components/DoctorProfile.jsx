...
const DoctorProfile = () => {
  ...
  const handleRespond = (feedbackId) => {
    ...
    setResponseText('');
    if (currentDoctor?._id) dispatch(fetchDoctorFeedback(currentDoctor._id));
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-4">
            <div className="w-20 h-20 bg-teal-100 rounded-xl flex items-center justify-center">
              <span className="text-3xl font-bold text-teal-600">{user?.firstName?.charAt(0)}{user?.lastName?.charAt(0)}</span>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div><p className="text-sm font-medium text-gray-500">Total Patients</p><p className="text-2xl font-bold text-gray-900 mt-1">{completedAppointments.length}</p></div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center mt-1"><p className="text-2xl font-bold text-gray-900">{parseFloat(doctorStats?.averageRating || 0).toFixed(1)}</p><span className="text-yellow-400 ml-1 text-xl">★</span></div>
              </div>
              <div className="w-12 h-12 bg-yellow-100 rounded-xl flex items-center justify-center"><svg className="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg></div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div><p className="text-sm font-medium text-gray-500">Total Likes</p><p className="text-2xl font-bold text-green-600 mt-1">{doctorStats?.likes || 0}</p></div>
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100">
              <div className="px-6 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Bio</h3></div>
              <div className="p-6"><p className="text-gray-700">{currentDoctor?.bio || 'No bio available'}</p></div>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100">
              <div className="px-6 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Contact Information</h3></div>
              <div className="p-6 space-y-3">
                <div className="flex items-center justify-between"><span className="text-gray-500">Email</span><span className="text-gray-900">{user?.email}</span></div>
                <div className="flex items-center justify-between"><span className="text-gray-500">Phone</span><span className="text-gray-900">{user?.phone || '-'}</span></div>
                <div className="flex items-center justify-between"><span className="text-gray-500">Experience</span><span className="text-gray-900">{currentDoctor?.experience || 0} years</span></div>
                <div className="flex items-center justify-between"><span className="text-gray-500">Consultation Fee</span><span className="text-gray-900 font-semibold">₦{currentDoctor?.consultation_fee?.toLocaleString()}</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'patients' && (
          <div className="space-y-4">
            ...
            {myAppointments.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">No patients yet</div>
            ) : (
          ...
          <div className="space-y-4">
            ...
            {doctorStats && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center"><p className="text-2xl font-bold">{doctorStats.total}</p><p className="text-sm text-gray-500">Total</p></div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center"><p className="text-2xl font-bold text-green-600">{doctorStats.likes}</p><p className="text-sm text-gray-500">Likes</p></div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center"><p className="text-2xl font-bold text-red-600">{doctorStats.dislikes}</p><p className="text-sm text-gray-500">Dislikes</p></div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center"><p className="text-2xl font-bold text-yellow-600">{doctorStats.pending}</p><p className="text-sm text-gray-500">Pending</p></div>
              </div>
            )}
            {doctorFeedback.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">No feedback yet</div>
            ) : (
              <div className="grid gap-4">
                      ...
                      <div className="ml-4">
                        {respondingTo === fb.id ? (
                          <div className="space-y-2">
                            <textarea value={responseText} onChange={(e) => setResponseText(e.target.value)} placeholder="Your response..." className="w-64 h-24 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                            <div className="flex space-x-2">
                              <button onClick={() => handleRespond(fb.id)} className="px-3 py-1 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700">Submit</button>
                              <button onClick={() => setRespondingTo(null)} className="px-3 py-1 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">Cancel</button>
                            </div>
                          </div>
                        ) : (
                          <button onClick={() => setRespondingTo(fb.id)} className="px-3 py-1 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">{fb.response ? 'Edit Response' : 'Respond'}</button>
                        )}
                        {fb.response && !respondingTo && <div className="mt-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg"><p className="font-medium">Your response:</p><p>{fb.response}</p></div>}
                      </div>
                    </div>
                  </div>
                ))}
